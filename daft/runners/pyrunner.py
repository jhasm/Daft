...
class LocalPartitionSet(PartitionSet[MicroPartition]):
    ...
    def _get_merged_vpartition(self) -> MicroPartition:
        ...
        return MicroPartition.concat(
            [part.partition() for id, part in ids_and_partitions]
        )
    ...
    def _get_preview_vpartition(self, num_rows: int) -> list[MicroPartition]:
        ...
        for _, mat_result in ids_and_partitions:
            ...
            if (
                part_len >= num_rows
            ):  # if this part has enough rows, take what we need and break
    ...
    def set_partition(
        self, idx: PartID, part: MaterializedResult[MicroPartition]
    ) -> None:
        self._partitions[idx] = part

    def set_partition_from_table(self, idx: PartID, part: MicroPartition) -> None:
        self._partitions[idx] = PyMaterializedResult(
            part, PartitionMetadata.from_table(part)
        )
    ...
    def __len__(self) -> int:
        return sum(
            len(partition.partition()) for partition in self._partitions.values()
        )

    def size_bytes(self) -> int | None:
        size_bytes_ = [
            partition.partition().size_bytes()
...
class PyRunnerIO(runner_io.RunnerIO):
    def glob_paths_details(
        ...
        file_format = (
            file_format_config.file_format() if file_format_config is not None else None
        )
...
class PyRunner(Runner[MicroPartition]):
    def __init__(self, use_thread_pool: bool | None) -> None:
        super().__init__()
        self._use_thread_pool: bool = (
            use_thread_pool if use_thread_pool is not None else True
        )
    ...
    def run_iter(
        ...
        if daft_execution_config.enable_aqe:
            adaptive_planner = builder.to_adaptive_physical_plan_scheduler(
            ...
            try:
                ...
                while True:
                    ...
                    while True:
                        if next_step is None:
                            ...
                            if (
                                ...
                                logger.debug(
                                    "Submitting task for execution: %s", next_step
                                )
                                ...
                                inflight_tasks_resources[next_step.id()] = (
                                    next_step.resource_request
                                )
                    ...
                    done_set, _ = futures.wait(
                        list(future_to_task.keys()), return_when=futures.FIRST_COMPLETED
                    )
    ...
    def _check_resource_requests(self, resource_request: ResourceRequest) -> None:
        ...
        if (
            resource_request.num_cpus is not None
            and resource_request.num_cpus > self.num_cpus
        ):
            raise RuntimeError(
                f"Requested {resource_request.num_cpus} CPUs but found only {self.num_cpus} available"
            )
        if (
            resource_request.num_gpus is not None
            and resource_request.num_gpus > self.num_gpus
        ):
            raise RuntimeError(
                f"Requested {resource_request.num_gpus} GPUs but found only {self.num_gpus} available"
            )
        if (
            resource_request.memory_bytes is not None
            and resource_request.memory_bytes > self.bytes_memory
        ):
            raise RuntimeError(
    ...
    def _can_admit_task(
        ...
        total_inflight_resources: ResourceRequest = sum(
            inflight_resources, ResourceRequest()
        )
        cpus_okay = (total_inflight_resources.num_cpus or 0) + (
            resource_request.num_cpus or 0
        ) <= self.num_cpus
        gpus_okay = (total_inflight_resources.num_gpus or 0) + (
            resource_request.num_gpus or 0
        ) <= self.num_gpus
    ...
    def build_partitions(
        ...
        return [
            PyMaterializedResult(
                part, PartitionMetadata.from_table(part).merge_with_partial(partial)
            )
