...
use crate::{
    compute::partition::{
        partition_task::Task, virtual_partition::VirtualPartitionSet, PartitionRef, PartitionTask,
    },
    executor::executor::Executor,
};

use super::partition_task_tree::{
...
#[derive(Debug)]
pub struct StreamingPartitionTaskScheduler<T: PartitionRef, E: Executor<T>> {
    state_root: Rc<PartitionTaskState<T>>,
    sorted_state: Vec<Rc<PartitionTaskState<T>>>,
    inflight_tasks: HashMap<usize, RunningTask<T>>,
    inflight_op_task_count: Vec<usize>,
...
impl<T: PartitionRef, E: Executor<T>> StreamingPartitionTaskScheduler<T, E> {
    pub fn from_state(
        state_root: Rc<PartitionTaskState<T>>,
        output_channel: std::sync::mpsc::Sender<DaftResult<Vec<T>>>,
        executor: Arc<E>,
    ...
    pub fn new(
        ...
        Self::from_state(state_root, output_channel, executor)
    }
        ...
        while !running_task_futures.is_empty() || self.has_inputs() {
                ...
                tokio::time::timeout(Duration::from_millis(10), running_task_futures.next()).await
            {
                let (task_id, result) = match result {
                    ...
                    Err(e) => {
                        self.output_channel.send(Err(e)).unwrap();
                        continue;
                    }
                };
                let running_task = self.inflight_tasks.remove(&task_id).unwrap();
                ...
                while self.state_root.num_queued_outputs() > 0 {
                    let outputs = self
                        .state_root
                        .outputs()
                        .iter()
                        .map(|v| v.borrow_mut().pop_front().unwrap())
                        .collect::<Vec<_>>();
                    // TODO(Clark): Await this once it's a buffered async queue, as a backpressure mechanism.
                    self.output_channel.send(Ok(outputs)).unwrap();
                }
            }
        }
        log::info!(
